<script lang="ts">
  ...
  /**
   * The staff door and what stands behind it, in one card. Signed out it is
   * the sign-in form with the published demo credentials; signed in it is the
  ...
  // Clearing the refusal the moment a field is edited, so a fixed typo does
  // not keep wearing the old error. Same behaviour as the other clients,
  // found in a live charter run on the Next.js form.
  $effect(() => {
    void email;
    void password;
    auth.clearError();
...
<section aria-label="Operator area" class="panel">
  <header class="panel-header">
    ...
    <p class="source">
      ...
      console, driven from Svelte this time.
    </p>
  </header>

  {#if auth.status === "unknown"}
    <p class="checking" role="status">Checking session…</p>
  {:else if auth.status === "signedIn" && auth.user}
    <div class="identity">
      <p class="who">
        Signed in as <strong>{auth.user.displayName}</strong>
        <span class="role">{auth.user.role}</span>
      </p>
      <button type="button" class="signout" onclick={() => auth.signOut()}>
        Sign out
      </button>
    </div>

    {#if auth.audit}
      <AuditTrail audit={auth.audit} onrefresh={() => auth.loadAudit()} />
    {/if}
    {#if auth.auditError}
      <p role="alert" class="error">{auth.auditError}</p>
    {/if}
  {:else}
    {#if auth.notice}
      <p role="status" class="notice">{auth.notice}</p>
    {/if}
    ...
    <div class="doors">
      ...
      >
        ...
        <div class="row">
          ...
          <PasswordField id="staff-password" bind:value={password} />
        </div>
      ...
      <aside class="demo">
        ...
        <button type="button" class="fill" onclick={fillDemoCredentials}>
