...
| --- | --- | --- | --- |
| Unit | 64 | Stubbed database and Stripe | Branching, mapping, status logic, guard clauses, password hashing, cookie attributes, rate-limit arithmetic |
...
| Production smoke | 25 | The deployed system, from outside, no credentials | That what *shipped* runs — including checks that only the newest build can satisfy |
