...
| 18 | Sentry had received no events in three weeks; the obvious reading was a broken integration | Two findings. The integration was correct — the API simply never throws, because every failure path returns `reply.code(4xx).send(...)` and Fastify's `onError` fires only on an unhandled exception. The real defect was that this was **unknowable**: `errorTrackingStatus()` derived `/health`'s answer from `process.env.SENTRY_DSN` being non-empty, so a typo'd key, a revoked key or a deleted project all reported `configured`. An empty dashboard and a misdirected tracker were the same picture, and the health check resolved the ambiguity in the wrong direction | `/health` reports actual initialisation, never env-var presence; `Sentry.init` wrapped so a malformed DSN logs and leaves the tracker off instead of taking down boot; one `api.startup` event emitted per boot so every deploy proves DSN, network and project routing end to end (ADR 18) | 2 unit tests — status stays `unconfigured` while only the env var is set, and status always agrees with what initialisation returned; plus the startup event itself, which is the only proof that can exist for the live leg |
