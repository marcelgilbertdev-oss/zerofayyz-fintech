...
- **A durable job queue in PostgreSQL** (2026-09-01, migration 008, ADR 15).
  ...
  pattern. First consumer: session-retention cleanup (hourly then; daily since ADR 22). Honest guarantee:
...
- **Probes and idle loops let the database sleep** (2026-09-13, [ADR 20](decisions/0020-probes-and-idle-loops-must-let-the-database-sleep.md)): Neon's free compute allowance was 81% spent by the 13th — on track to suspend the ledger on the 16th — because Render's every-few-seconds health check queried the database and the job worker polled every 30 seconds. Render and the canary now hit a dependency-free `/api/v1/live`; the worker sleeps until the next job is due, is woken by in-process enqueues, and polls at most hourly as a safety net. Three tests pin it: liveness never touches the database, an enqueue wakes an idle worker at once, and a delayed job runs when due rather than at the cap.
- **A connection timeout learned to cover a cold start** (2026-09-17, [ADR 21](decisions/0021-a-connection-timeout-must-cover-a-cold-start.md)): letting the database sleep made a setting chosen for a database that never slept newly reachable. The connection pool allowed 1500ms to open a connection; a Neon resume measures about 1.7 seconds, so the first visitor after an idle spell landed either side of the line and Sentry raised its first two error issues — both on the ADR 20 release itself. The ceiling is now ten seconds and a connection is retried once, but only when the failure happened while establishing it, so a statement that did reach the database is never run twice. Five tests pin which errors qualify and which are raised immediately.
- **Recurring work runs as rarely as its retention allows** (2026-09-17, [ADR 22](decisions/0022-recurring-work-runs-as-rarely-as-its-retention-allows.md)): read from the Neon console the same morning, the compute allowance stood at 88.52 of 100 with 0.97 a day still burning — on course to suspend the ledger around the 29th. Half of that was the session-cleanup chain waking the database every hour to run a thirty-day delete. The chain is now keyed by UTC day and the worker's safety cap is six hours instead of one, so the cap is never the waker; projected landing is 92–93 on the 30th.
