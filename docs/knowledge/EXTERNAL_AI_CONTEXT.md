...
**Shape:** one API, three frontends, one database, one independent checker — plus two consumers outside the repository: a Manifest V3 extension (endpoint-pulse) and a customer receipt portal built entirely on Supabase (receipt-portal, ADR 17), live at receipt-portal-one.vercel.app with a sync-payments Edge Function that consumes this API.
