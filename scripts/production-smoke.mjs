...
// The platform's central claim is that one API is consumed unmodified by three
// clients, and from inside any one of them that was unverifiable — the footers named
// the client you were on and offered no way to reach its siblings.
//
// Only the dashboard can be checked from here. It is server-rendered, so its markup
// arrives in the HTML; the Vue and Svelte clients ship a shell and render on the
// client, and SvelteKit lazy-loads the very chunk the markup lives in. A smoke test
// that reverse-engineered a bundler's chunk graph would break on the next framework
// upgrade for reasons having nothing to do with the link it claims to guard. Those two
// are covered by component tests instead, which is where rendered output belongs.
await check("the dashboard points at the other two clients", async () => {
  const html = await (await fetch(WEB, { signal: AbortSignal.timeout(TIMEOUT_MS) })).text();

  for (const href of ["fintech-vue.vercel.app", "fintech-svelte.vercel.app"]) {
    assert(html.includes(href), `the dashboard does not link to ${href}`);
  }

  return "both sibling clients reachable from the dashboard";
});
